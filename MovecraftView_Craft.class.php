  ...
  class MovecraftView_Craft{
    ...
    function getStringByKey($key,$order = "asc"){
      ...
      if (is_array( $val)){
        ...
        if ($key != MV_KEY_FLY_BLOCKS || $key != MV_KEY_MOVE_BLOCKS){
        ...
        foreach ($val as $kv => $v){
          ...
          if (is_array($v)){
            $val1[$kv] = "<i>".(function_exists("getBlockByKey")?getBlockByKey($kv):$kv).":</i> ".$v[0]." - ".$v[1]." %";
            $tmp .= $val1[$kv];
          }else{
    ...
    function setStringByKey($key,$val){
      if ($key == MV_KEY_NAME){
        ...
        $key == MV_KEY_MOVE_BLOCKS && is_array($val)
      ){
        foreach ($val as $k => $v){
          if (is_array ($v)){
            foreach ($v as $k1 => $v1){
