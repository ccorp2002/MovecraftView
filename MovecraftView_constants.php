<?php
  ...
  define ('MV_KEY_PHASE_BLOCKS','passthroughBlocks');
  define ('MV_KEY_FLY_BLOCKS','flyblocks');
  define ('MV_KEY_MOVE_BLOCKS','moveblocks');
  ...
  define (
    "KEYS", serialize (
      array (
        MV_KEY_NAME, MV_KEY_MIN_SIZE, MV_KEY_MAX_SIZE, MV_KEY_SPEED, MV_KEY_TRY_NUDGE,
        ...
        MV_KEY_FLY_BLOCKS, MV_KEY_ALLOWED_BLOCKS, MV_KEY_FORBIDDEN_BLOCKS, MV_KEY_MOVE_BLOCKS,MV_KEY_CAN_FLY
      )
    )
  );
  ...
  if (!defined('MV_LABEL_PHASE_BLOCKS')) define ('MV_LABEL_PHASE_BLOCKS','Passthrough/Phase Blocks:');
  if (!defined('MV_LABEL_FLY_BLOCKS')) define ('MV_LABEL_FLY_BLOCKS','Lift/Fly blocks:');
  if (!defined('MV_LABEL_MOVE_BLOCKS')) define ('MV_LABEL_MOVE_BLOCKS','Movement/Engine blocks:');
  ...
  if (!defined('MV_TITLE_SPEED')) define ('MV_TITLE_SPEED','Maximum speed while holding right mouse button (not cruise)');
  if (!defined('MV_TITLE_TRY_NUDGE')) define ('MV_TITLE_TRY_NUDGE','Try nudge');
  if (!defined('MV_TITLE_CAN_FLY')) define ('MV_TITLE_CAN_FLY','Flight/Space Capable Craft');
  if (!defined('MV_TITLE_CAN_CRUISE')) define ('MV_TITLE_CAN_CRUISE','Can use cruise');
  if (!defined('MV_TITLE_CRUISE_SKIP_BLOCKS')) define ('MV_TITLE_CRUISE_SKIP_BLOCKS','Count of skipped blocks each move with cruise');
  ...
  if (!defined('MV_TITLE_HARVEST_BLOCKS')) define ('MV_TITLE_HARVEST_BLOCKS','Block which will be destroyed when craft moves on their position');
  if (!defined('MV_TITLE_PHASE_BLOCKS')) define ('MV_TITLE_PHASE_BLOCKS','Block which are able to be flown through by a craft, without destroying the blocks.');
  if (!defined('MV_TITLE_FLY_BLOCKS')) define ('MV_TITLE_FLY_BLOCKS','Blocks necessary for a craft to function/exist/be piloted');
  if (!defined('MV_TITLE_MOVE_BLOCKS')) define ('MV_TITLE_MOVE_BLOCKS','Blocks necessary to fly/move');
  if (!defined('MV_TITLE_ALLOWED_BLOCKS')) define ('MV_TITLE_ALLOWED_BLOCKS','List of allowed/pilotable blocks.');
  if (!defined('MV_TITLE_FORBIDDEN_BLOCKS')) define ('MV_TITLE_FORBIDDEN_BLOCKS','List of forbidden blocks. Craft can not be activated while it is connected to them.');
  ...
  function getHeadByKey($key){
      switch ($key){
        ...
        case MV_KEY_PHASE_BLOCKS:
          return MV_LABEL_PHASE_BLOCKS; 
        case MV_KEY_FLY_BLOCKS:
          return MV_LABEL_FLY_BLOCKS; 
          case MV_KEY_MOVE_BLOCKS:
            return MV_LABEL_MOVE_BLOCKS; 
